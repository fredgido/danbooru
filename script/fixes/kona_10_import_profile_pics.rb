...
DIRECTORY = ENV.fetch("DIRECTORY", "profile_pics")
...
def process_file(file_path)
  ...
  uploaded_file = MediaAsset.upload!(File.open(file_path), role: "avatar")
  media_asset = MediaAsset.find_by(md5: file_md5)
  if media_asset.nil?
  ...
  profile_picture = ProfilePicture.insert_all([{
                                                 user_id: file_name.split(".")[0].to_i,
                                                 created_at: file_modified_at,
                                                 updated_at: file_modified_at,
                                                 source_media_asset_id: media_asset.id,
                                                 avatar_media_asset_id: media_asset.id,
                                                 height: media_asset.image_height,
                                                 width: media_asset.image_width,
                                                 top: 0,
                                                 left: 0,
                                               }]).rows.flatten[0]

  puts "Saved profile pic with id ##{profile_picture}."
